...
def plot_data_and_smooths(series, smooth_boots, ID):
    ...
    p1 = (
        ggplot(data, aes(x="Sample Index", y="Value_clipped", color="Sample"))
    ...
    p1 = (
        ...
        + geom_line(smooth_boots[smooth_boots["ID"]==ID], 
                    aes(x="Sample Index", y="Value", group="Group", color="Sample"), 
                    alpha=0.2)
    )
    return p1
...
def plot_hists(count_estimates, estimates, ID):
    ...
    boot_estimates[""] = "Bootstrap"
    p2 = (ggplot(boot_estimates, aes(x="Peaks/Bands Estimate", fill=""))
        + geom_histogram(binwidth=1, center=0, alpha=0.5)
        + facet_wrap("~Variable", ncol=1, scales='free_x')
        + my_theme)
    p2 = (p2 
        + geom_vline(
            data=count_estimates[(~boot_idx) & id_idx],
            mapping=aes(xintercept="Peaks/Bands Estimate", color="Method"),
...
if __name__ == "__main__":
    ...
    count_estimates = pd.concat(get_clean_estimates(count_estimates_filename))
    count_estimates["Method"] = count_estimates["Type"]
    count_estimates["Method"] = count_estimates["Type"].replace({
        "estimate": "Estimate"
    })
    ...
    series["Sample"] = "Data"
    smooth_boots["Sample"] = "Bootstrap smooths"

    smooth_boots_thin_factor = 8
    smooth_boots_thinned = smooth_boots[(np.asarray(smooth_boots["Boot Index"], dtype=int) % smooth_boots_thin_factor) == 0]

    for ID in ggi.GG_SR_IDS:
        id = f"Specimen {ID}"
        p1 = plot_data_and_smooths(series, smooth_boots_thinned, id)
        ...
        plt.tight_layout()
        plt.savefig(f"out/GG{ID}_estimates{count_estimates_filename.replace(".", "").replace("/", "_")}.pdf")
